...
import { BookOpen, FileText, Film, Loader2, Pencil, Plus, Search, Trash2, Upload } from "lucide-react";
...
export default function TreinamentosPage() {
  ...
  async function save() {
    ...
    try {
      ...
      if (file) {
        ...
        upload = await uploadRes.json(); if (!uploadRes.ok) throw new Error((upload as unknown as { error?: string }).error || "Erro no upload");
      }
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      {isAdmin && <Button onClick={() => openForm()}><Plus className="mr-2 h-4 w-4" /> Novo conteúdo</Button>}
    </div>
    ...
    <Dialog open={formOpen} onOpenChange={setFormOpen}><DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>{editing ? "Editar treinamento" : "Novo treinamento"}</DialogTitle><DialogDescription>Vídeos ficam no volume de uploads; POPs devem ser enviados como Documento PDF.</DialogDescription></DialogHeader><div className="grid gap-4 py-2">
      ...
      <div className="grid gap-3 sm:grid-cols-[1fr_120px]"><div className="space-y-2"><Label>Categoria *</Label><Select value={formCategory} onValueChange={(value) => value && setFormCategory(value)}><SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger><SelectContent>{categories.map((c) => <SelectItem key={c.id} value={c.id}>{c.nome}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label>Ordem</Label><Input type="number" value={ordem} onChange={(e) => setOrdem(e.target.value)} /></div></div>
      ...
      {!editing && <div className="space-y-2"><Label>Vídeo ou documento PDF *</Label><Input type="file" accept="video/mp4,video/webm,video/ogg,video/quicktime,application/pdf" onChange={(e) => setFile(e.target.files?.[0] ?? null)} /><p className="text-xs text-muted-foreground">Vídeos até 500 MB; PDFs até 50 MB. O tipo é identificado automaticamente.</p></div>}
