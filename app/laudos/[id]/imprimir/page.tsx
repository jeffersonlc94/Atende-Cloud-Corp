...
export default function TechnicalReportPrintPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="min-h-screen bg-gray-200"><div className="sticky top-0 z-10 flex flex-wrap items-center justify-between gap-2 border-b bg-background p-4 print:hidden"><Link href="/laudos" className={buttonVariants({ variant: "ghost" })}><ArrowLeft className="mr-2 h-4 w-4" /> Voltar</Link><div className="flex gap-2"><Link href="/laudos" className={buttonVariants({ variant: "outline" })}><Pencil className="mr-2 h-4 w-4" /> Editar</Link><Button variant="outline" onClick={() => window.print()}><Printer className="mr-2 h-4 w-4" /> Imprimir</Button><Button disabled={generating} onClick={generatePdf}>{generating ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <FileDown className="mr-2 h-4 w-4" />}Gerar PDF</Button></div></div><div className="py-8 print:py-0"><TechnicalReportPrint report={report} /></div></div>;
}
