...
export type TechnicalReportPrintData = {
  ...
  fotos?: Array<string | { url: string; legenda?: string | null }> | null;
...
export function TechnicalReportPrint({ report, id = "technical-report-print", layout = "classico" }: { report: TechnicalReportPrintData; id?: string; layout?: "classico" | "fotos" }) {
  ...
  const photos = (report.fotos || []).map((photo) => typeof photo === "string" ? { url: photo, legenda: "" } : { url: photo.url, legenda: photo.legenda || "" }).slice(0, 3);
  return <div id={id} className="mx-auto w-full max-w-[210mm] bg-white p-5 text-black print:p-4" style={{ fontFamily: "Arial, Helvetica, sans-serif", fontSize: "15px" }}>
    ...
    {layout === "fotos" && photos.length > 0 && <section data-technical-report-photos className="mt-3 break-before-page rounded-lg border border-black"><h2 className="border-b border-black px-3 py-1 text-center text-[0.78em] font-bold uppercase">Registro fotográfico</h2><div className={`grid gap-3 p-3 ${photos.length === 1 ? "grid-cols-1" : "grid-cols-2"}`}>{photos.map((photo, index) => <figure key={`${photo.url}-${index}`} className={photos.length === 3 && index === 2 ? "col-span-2" : ""}><img src={photo.url} alt={`Registro fotográfico ${index + 1}`} className={`mx-auto max-w-full object-contain ${photos.length === 1 ? "h-[120mm]" : "h-[82mm]"}`} /><figcaption className="mt-2 text-center text-[0.7em]"><span className="block font-bold">Foto {index + 1}</span>{photo.legenda && <span className="mt-0.5 block">{photo.legenda}</span>}</figcaption></figure>)}</div></section>}
