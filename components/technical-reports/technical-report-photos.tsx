...
export function TechnicalReportPhotos({ photos, onChange }: { photos: string[]; onChange: (photos: string[]) => void }) {
  ...
  return <div className="space-y-3 rounded-xl border border-dashed p-4" tabIndex={0} onPaste={(event) => { const images = Array.from(event.clipboardData.files).filter((file) => file.type.startsWith("image/")); if (images.length) void upload(images); }}>
    ...
    {photos.length > 0 && <div className="grid gap-3 sm:grid-cols-3">{photos.map((url, index) => <div key={`${url}-${index}`} className="relative overflow-hidden rounded-lg border bg-muted"><img src={url} alt={`Foto ${index + 1}`} className="h-40 w-full object-contain" /><Button type="button" variant="destructive" size="icon-sm" className="absolute right-2 top-2" onClick={() => onChange(photos.filter((_, current) => current !== index))}><Trash2 className="h-4 w-4" /></Button><p className="border-t bg-background px-2 py-1 text-xs">Foto {index + 1}</p></div>)}</div>}
  </div>;
}
